...
test('TOPIC CARD: Renders correct default text when no questions supplied', () => {
...
test('TOPIC CARD: Renders a topic on the supplied info', () => {
  const container = document.createElement('div');
  //Create dummy data
  const topic = {
    title: 'Test Title',
...
//Perform required tests
  const heading = getByTestId(container, "title");
  expect(heading.textContent).toEqual(topic.title);   
  const authorName = getByTestId(container, "author");
